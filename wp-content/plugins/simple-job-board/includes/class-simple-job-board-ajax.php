...
class Simple_Job_Board_Ajax {
    ...
    public function fetch_quick_job() {
        ...
        if (!is_user_logged_in() || !current_user_can('manage_options')) {
            // You can customize the error message or redirect the user to a login page
        ...
        // Check if the user has permission to view the requested post
        if (!current_user_can('read_post', $job_id)) {
            wp_die('Unauthorized Access', 'Unauthorized', array('response' => 403));
        }
    
        // Get the post using proper WordPress functions
        $post = get_post($job_id, OBJECT);
    
        // Check if the post is valid and has the necessary conditions
        if (!$post || is_wp_error($post) || ($post->post_status !== 'publish' && !current_user_can('edit_post', $job_id))) {
        ...
        <div class="popup-outer">
            ...
            <div class="job-description" id="job-desc">
                <?php
                echo apply_filters('popup_post_content', wp_kses_post(nl2br($post->post_content)));
                ?>
            </div>
